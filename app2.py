...
def get_leaderboard_stats():
    ...
    try:
        ...
        session_stats = results_df.groupby(['User Name', 'Date', 'Exam', 'Section', 'Topic']).agg({
            'Result (Correct/Wrong)': lambda x: (x == 'Correct').sum()
        }).reset_index()
        
        session_stats.rename(columns={'Result (Correct/Wrong)': 'Score'}, inplace=True)
        session_stats['Total Questions'] = 10  # Assuming 10 questions per quiz
        ...
        user_stats['Total Questions'] = user_stats['Total Attempted'] * 10
        user_stats['Overall Accuracy'] = (user_stats['Total Correct'] / user_stats['Total Questions'] * 100).round(2)
