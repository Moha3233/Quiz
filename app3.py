...
def update_timer():
    ...
    if (st.session_state.quiz_started and 
        ...
        elapsed_since_last_update = (current_time - st.session_state.last_update).total_seconds()
        st.session_state.remaining_time -= elapsed_since_last_update
...
def display_timer():
    ...
    if st.session_state.quiz_started and not st.session_state.time_up:
        ...
        # Create a dynamic timer display
        ...
        if st.session_state.remaining_time > 0:
            ...
            timer_placeholder.error("⏰ Time's up! Quiz completed automatically.")
